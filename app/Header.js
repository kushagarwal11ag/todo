...
const Header = () => {
	...
	const { todos } = context;
	const { getUserDetails } = userNameContext;

	const [credentials, setCredentials] = useState({ userName: "User" });
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			setCredentials({ userName: data.name });
			setImage(data.image);
		};
		fetchData();
	}, []);
	...
	const tasksLeft = () => {
		let count = 0;
		todos.map((todo) => {
			todo.completed === false ? count++ : null;
		});
		return count > 0
			? count > 1
				? `${
	...
	return (
		<>
			<div className="container header my-5">
				<div>
					<img id="img" src={image} alt="User Profile Image" />
				</div>
				<h1>{tasksLeft()}</h1>
			</div>
		</>
